...
uint8_t STEPPER::Manual_Set(void)
{
	...
	if (key)
	{
		switch (key)
		{
			case KEY_UP:	y += steplen; break;
			case KEY_DOWN:	y -= steplen; break;
			case KEY_LEFT:	x -= steplen; break;
			case KEY_RIGHT:	x += steplen; break;
			case KEY_OK:
            	lazerON();
                delay_ms(500);
                lazerOFF();
...
void STEPPER::Draw_Circle(uint32_t x, uint32_t y, uint32_t r)
{
    ...
    for(i = -(double)radius; i < radius; i += steplen){//上半
        Move_To((uint32_t)(x + i * scale), (uint32_t)(y + sqrt(radius * radius - i * i) * scale));
    }
    for(i = radius; i > -(double)radius; i -= steplen){//下半
        Move_To((uint32_t)(x + i * scale), (uint32_t)(y - sqrt(radius * radius - i * i) * scale));
    }
...
void STEPPER::Draw_Pixel(uint32_t x, uint32_t y, uint16_t size, uint8_t proportion)
{
    ...
    for(uint32_t i = size; i > 0; i--)
    {
        if (err > 1) {
            ...
            err += ratio;
            if(sta) {
                Draw_Line(x, y + i, x + size, y + i);
            }
            else {
                Draw_Line(x + size, y + i, x, y + i);
            }
            sta = !sta;
        }
    }
...
void STEPPER::Draw_Char(uint32_t x, uint32_t y, uint8_t ch, uint16_t size, uint8_t proportion)
{
    ...
    {
        if(size==12)temp=asc2_1206[ch][i];
        else if(size==16)temp=asc2_1608[ch][i];
        else if(size==24)temp=asc2_2412[ch][i];
        else return;
        for(uint8_t j = 0; j < 8; j++)
        {
            if (temp & 0x80) Draw_Pixel(x, y, 128, proportion);
            temp <<= 1;
            y--;
            if((y0 - y) == size)
            {
...
void STEPPER::Draw_AvengersLogo(void)
{
  ...
  STEPPER::Draw_Line(530, 500, 3250, 6000);//A-B
  STEPPER::Draw_Line(3250, 6000, 4100, 6000);//B-D
  STEPPER::Draw_Line(4100, 6000, 4100, 1550);//D-E
  STEPPER::Draw_Line(4100, 1550, 3250, 1550);//E-F
  STEPPER::Draw_Line(3250, 1550, 3250, 2100);//F-G
  STEPPER::Draw_Line(3250, 2100, 2300, 2100);//G-H
  STEPPER::Draw_Line(2300, 2100, 1580, 500);//H-I
  STEPPER::Draw_Line(1580, 500, 530, 500);//I-A

  STEPPER::Draw_Line(3300, 4500, 3300, 2900);//J-L
  STEPPER::Draw_Line(3300, 2900, 2640, 2900);//L-K
  STEPPER::Draw_Line(2640, 2900, 3300, 4500);//K-J

  STEPPER::Draw_Line(3300, 3270, 4100, 2500);//M-N
  STEPPER::Draw_Line(4100, 2500, 3250, 1700);//N-O
